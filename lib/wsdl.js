          ...
          if (schema) {
            ...
            if (childParameterTypeObject && childParameterTypeObject.$type && (childParameterTypeObject.$type.indexOf('xsd') === -1)) {
              ...
              if(Array.isArray(child)) {
                ...
                // for arrays, we need to declare the namespace if already not defined
                value = self.objectToXML(child, name, childNamespace, childXmlns, true, childXmlnsAttrib, completeChildParameterTypeObject, ancXmlns);
              } else {
  ...
  p.onopentag = function(node) {
    ...
    if (top) {
      ...
      if (name === 'definitions') {
        ...
        stack.push(root);
      } else if (name === 'schema' || name === 'schemaEntry') {
